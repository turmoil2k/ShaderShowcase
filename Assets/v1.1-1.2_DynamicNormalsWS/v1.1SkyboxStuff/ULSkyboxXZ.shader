Shader "Unlit/ULSkyboxXZ"
{
    Properties
    {
        _MainTex ("Stars Tex", 2D) = "white" {}     //x y z w  = 4 4 0 0
        _CloudTex ("Cloud Tex", 2D) = "white" {}    //x y 0 0
        ...
        [HDR]_ColorSun("Sun Col", color) = (1,1,1,1)
        [HDR]_ColorCloud("Cloud Color (Alpha included)", color) = (1,1,1,1)

        _Vector("XY>Star Noise Vector | Z>STAR SIZE", vector) = (0.02,0.01,15,0.05)
        ...
        _DistStrength("_DistStrength", Range(-5,5)) = 1
        _NoisePower("_NoisePower", Range(-5,5)) = 2
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            float SkyMask(float pos, float power)
            {
                return pow(max(pos,0),power);
            }
            ...
            fixed4 frag (v2f i) : SV_Target
            {
                ...
                //float skyMask = SkyMask(worldPos.y,2);
                
                //_MainTex_ST.zw += (_Time.y * _Vector.xy);
                //_RandNoiseTex_ST.z += (_Time.y * _Vector.w);

                //tilling & offset / UV SETTERS
                float2 skyUVST = Unity_TilingAndOffset_float(worldPos.xz,_MainTex_ST.xy,_MainTex_ST.zw);
                float2 cloudUVST = Unity_TilingAndOffset_float(worldPos.xz,_CloudTex_ST.xy,_CloudTex_ST.zw);
                float2 noiseUVST = worldPos.xz;
                noiseUVST.x += (_Time.y * _Vector.w);
                //noiseUVST = _Vector.w * (noiseUVST / worldPos.y) + (_Vector.w * _Time.y);

                //Sampling the Texture && Stars Movement
                skyUVST = (skyUVST / worldPos.y) + (0.01 * _Time.y);
                float4 starTex = saturate(1 - tex2D(_MainTex,skyUVST));// (i.uv + (_Vector.xy * _Time.y)));

                float2 distCol = tex2D(_DistTex,worldPos.xz)*_DistStrength;
                float noiseCol = pow(tex2D(_RandNoiseTex,noiseUVST + distCol.xy),_NoisePower);//control 2 for amt of stars
                //Stars Creation change z for brightness power    //sin(_Time.y/2)|* (noiseCol.r))      //4 IS THE POWER OF HORIZON / starting line of horizon for stars
                starTex = pow(saturate(starTex),_Vector.z) * (_ColorC  * noiseCol) * SkyMask(worldPos.y,1);//(sin(_Time.y*4) * 10 + _Vector.z)) * _ColorC;
                ...
                cloudUVST = (cloudUVST / worldPos.y) + (0.01 * _Time.y);
                //float4 colorBand = lerp(float4(0,0,0,1),float4(1,1,1,1),i.uv.y - 0.95)*lerp(float4(1,1,1,1),float4(0,0,0,1),i.uv.y - 0.95);
                float4 cloudsTex = saturate(tex2D(_CloudTex,cloudUVST) * 1) * (_ColorCloud);
                //clip(cloudsCol-0.2);
                //return float4(worldPos.yyy * _ColorCloud.xyz,1);

                float cloudMask = SkyMask(worldPos.y,4) * cloudsTex.a;//float(pow(max(worldPos.y,0),3)) * cloudsTex.a;
                ...
                //return skyMask;
                //final clouds go into the brackets!!!          //check alpha of clouds
                return finalColorLerp + starTex + finalSuns + finalClouds;//+ 1;// + cloudsTex;
